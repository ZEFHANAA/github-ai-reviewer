...
@section('title', __('Repository health, explained'))
    ...
    <section class="mx-auto max-w-6xl px-6 py-20 sm:py-28 lg:px-8 lg:py-36">
        <div class="max-w-3xl">
            <p class="mb-6 inline-flex items-center gap-2 rounded-full border border-emerald-300/20 bg-emerald-400/10 px-3 py-1 text-sm font-medium text-emerald-200">
                ...
                {{ __('Deterministic checks. Practical guidance.') }}
            </p>

            <h1 class="text-4xl font-semibold tracking-tight text-balance text-white sm:text-6xl">
                {{ __('Analyze a public GitHub repository with confidence.') }}
            </h1>

            <p class="mt-7 max-w-2xl text-lg leading-8 text-slate-300 sm:text-xl">
                {{ config('app.name') }} {{ __('turns bounded repository signals into an explainable health report. Deterministic checks own scores; optional AI only clarifies what to improve next.') }}
            </p>
        </div>
        ...
        <div class="mt-14 grid gap-5 md:grid-cols-3">
            <article class="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
                ...
                <h2 class="text-lg font-semibold text-white">{{ __('Explainable scores') }}</h2>
                <p class="mt-2 text-sm leading-6 text-slate-300">{{ __('Clear category scores supported by identifiable checks and findings.') }}</p>
            </article>

            <article class="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
                <div class="mb-5 grid size-10 place-items-center rounded-lg bg-indigo-400/15 text-indigo-200">02</div>
                <h2 class="text-lg font-semibold text-white">{{ __('Practical guidance') }}</h2>
                <p class="mt-2 text-sm leading-6 text-slate-300">{{ __('Recommendations focused on documentation, testing, security, structure, code quality, and Git practices.') }}</p>
            </article>

            <article class="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
                <div class="mb-5 grid size-10 place-items-center rounded-lg bg-indigo-400/15 text-indigo-200">03</div>
                <h2 class="text-lg font-semibold text-white">{{ __('Responsible AI') }}</h2>
                <p class="mt-2 text-sm leading-6 text-slate-300">{{ __('AI explains and prioritizes; deterministic checks remain the foundation for scoring.') }}</p>
            </article>
        </div>

        <div class="mt-14 max-w-3xl rounded-2xl border border-indigo-300/15 bg-indigo-400/10 p-6 sm:p-8">
            <h2 class="text-xl font-semibold text-white">{{ __('Start with a public GitHub repository') }}</h2>
            <p class="mt-2 text-sm leading-6 text-indigo-100">{{ __('Enter a canonical repository page URL. We will validate it before repository analysis is available.') }}</p>

            <form action="{{ route('repositories.submit') }}" method="POST" class="mt-6" data-analyze-form>
                @csrf

                <label for="repository_url" class="text-sm font-medium text-white">{{ __('GitHub repository URL') }}</label>
                ...
                <p id="repository_url_help" class="mt-2 text-sm text-indigo-100/80">{{ __('Example: https://github.com/laravel/laravel') }}</p>
